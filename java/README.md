...
Deze applicatie hoort bij [woozer/ci](https://github.com/woozer/ci). De onderstaande build- en testcommando's werken vanuit deze map; de GitLab-pipeline en deployment vereisen de [lokale demo-inrichting](https://github.com/woozer/ci/blob/main/installation.md).
...
Vóór publicatie controleert OWASP Dependency-Check de Maven-dependencies en voert de lokale SonarQube Community Build broncodeanalyse met een quality gate uit. Dependency-Check draait ook in merge requests; Sonar analyseert de beschermde `main` en releases. Een afgekeurde scan blokkeert publicatie van backend en UI. Je hoeft zelf geen API-key aan te vragen; zie de [scanhandleiding](https://github.com/woozer/ci/blob/main/docs/scanners.md) voor rapporten en beperkingen, waaronder de nog niet actieve npm-audit; de sample levert wel coverage-rapporten.

[.gitlab-ci.yml](.gitlab-ci.yml) neemt het gedeelde formulier en één centrale [java-service.yml](https://github.com/woozer/ci/blob/main/pipelines/java-service.yml) op. CI-scripts en jobvolgorde staan in die bibliotheek. Helm leest eerst `environment/cluster/<cluster>.yaml` en daarna `environment/user/<profile>.yaml`. Het profiel `two-replicas` laat een aanpassing zien. Toegangsgegevens blijven in GitLab.

Formulier en pipeline gebruiken beide componentversie `1.0.0`. De YAML-anchor `ci_strategy` geeft diezelfde versie door als `library-ref`. Een bibliotheekupgrade verandert beide `ref`-waarden via een merge request; zie de [versieafspraken](https://github.com/woozer/ci/blob/main/docs/component-versions.md).
...
Maven-packages gaan naar GitLab; images en charts naar lokale Artifactory. Ook een release wordt in dev gevalideerd. Toekomstige productiedeployment via Argo CD moet de bestaande release-image-digests gebruiken zonder opnieuw te bouwen. Zie [pipelinekeuzes](https://github.com/woozer/ci/blob/main/docs/pipeline-options.md) en [releasebeleid](https://github.com/woozer/ci/blob/main/docs/releases.md).
