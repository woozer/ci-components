...
The optional [organization profile](profiles/organization.yml) supplies a separate image for each job and common operational defaults through those inputs. Applications can override the profile's inputs or consume individual modules directly. See [defaults and profiles](docs/defaults.md) for the ownership and override rules.
