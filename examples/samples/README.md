...
Publicatietests maken echte artifacts. Maven-packages en testreleases staan in `ci-samples`; images en charts staan onder `docker-local/root-ci-samples/`. De releasevoorbeelden reserveren unieke tags en voeren normaal handmatige jobs uitsluitend in deze geïsoleerde testopstelling automatisch uit. Tags en gepubliceerde versies blijven bewaard. De jobs wijzigen geen releases of deployments van `hello-world`. De Sonar-sample gebruikt het eigen SonarQube-project `ci-samples`.
