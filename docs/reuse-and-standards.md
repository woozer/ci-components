...
De TODO-modules zijn op drie plaatsen samengevoegd: `sonar` verzorgt analyse en de native quality gate, `fortify` combineert scan en beleidscontrole, en `image-scan` maakt ook het CycloneDX-SBOM. Daarmee vervallen drie losse componenten en de eigen Sonar-pollingcode. Dit is een keuze voor eenvoud, geen voorgeschreven industriestandaard. De modules blijven in `modules/todo/` totdat hun echte integratie is gevalideerd.
