...
Versie `2.0.0` verwijdert de oude pipelinebestanden uit deze bibliotheek na de verhuizing naar `ci-pipelines`. Daarom is dit een nieuwe majorversie. De losse taakcomponenten blijven beschikbaar. De versie voegt ook JaCoCo-diffweergave, JUnit voor Dependency-Check en scannerreacties toe. Werk voor die reacties de scanimage bij met de centrale Python-helpers en stel de beschreven rapportagetokens in; de lokale installer verzorgt beide. Zie [de scanhandleiding](scanners.md).
